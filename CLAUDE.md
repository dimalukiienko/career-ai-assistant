...
`encryptor`, `tokens` (TokenStore), `sessions` (SessionStore). These three are **interfaces
with in-memory/local impls** chosen here. Swapping to Supabase later = add new impls and
change this one file; nothing else imports a concrete store.
...
verified at `/callback`. The callback exchanges the code, **encrypts the refresh token via
the Encryptor**, and stores it.
...
self-describing (`v1:kms:...` / `v1:local:...`). `getAuthedClient` (`src/google/client.ts`)
decrypts the refresh token, builds the OAuth2 client, and persists rotated access tokens via
the `tokens` event.

## v1 constraints (deliberate, documented in README)

- Stores are **in-memory** → tokens/sessions are per-instance and lost on cold start. Run
  Cloud Run at `min/max-instances=1` until a shared store (Supabase) lands.
