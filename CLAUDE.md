...
soft-close via `ended_at`). `0003_token_usage.sql`: `token_usage` (one running-total row per
profile — lifetime model-token spend; incremented atomically via the `add_token_usage` RPC,
covering both agent turns and summarization). `SupabaseTokenStore`/`SupabaseSessionStore`/
`SupabaseUsageStore` map `uid` (Telegram id) → profile → row; token `delete` is a soft revoke
and `get` ignores revoked rows. A partial unique index (`profile_id where ended_at is null`)
enforces **one active session per profile**.
